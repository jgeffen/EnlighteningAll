...
class Log implements Interfaces\Action {
	...
	/**
	 * @param null|Members\Log $log
	 * @param Membership       $member
	 * @param Requests\Log     $action
	 */
	public function __construct(?Members\Log $log, Membership $member, Requests\Log $action = Requests\Log::ADD) {
		$this->log    = $log;
		$this->member = $member;
		$this->action = $action;
	}
	 ...
	 * @return $this
	 */
	public function setData(Types\Log $type, ?Interfaces\TableEnum $table_name = NULL, ?int $table_id = NULL, ?Types\Column $table_column = NULL, ?string $filename = NULL, ?string $notes = NULL): self {
		$this->type         = $type;
	...
	private function add(): bool {
		return Database::Action("INSERT INTO `member_logs` SET `type` = :type, `member_id` = :member_id, `table_name` = :table_name, `table_id` = :table_id, `table_column` = :table_column, `filename` = :filename, `notes` = :notes, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
	...
	private function remove(): bool {
		return Database::Action("DELETE FROM `member_logs` WHERE `id` = :id AND `member_id` = :member_id", array(
	...
	/**
	 * @return null|Members\Log
	 */
	private function getLog(): ?Members\Log {
		return $this->log;
	}

<?php
	...
	*/
	...
	enum BeadColors: string {
		case STRAIGHT = 'Straight';
		case     PINK = 'Same Sex';
		case    WHITE = 'Single or Open Relationship';
		...
		public static function lookup(?string $value): ?self {
			return match ($value) {
				self::STRAIGHT->getValue() => self::STRAIGHT,
				self::PINK->getValue()     => self::PINK,
				self::WHITE->getValue()    => self::WHITE,
				self::PURPLE->getValue()   => self::PURPLE,
				self::BLACK->getValue()    => self::BLACK,
				default                    => NULL
			};
		}

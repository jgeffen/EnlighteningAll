...
try {
	// Check Logged In
	if (TravelAffiliateMembership::LoggedIn(FALSE)) throw new Exception('You must be logged in to perform this action.');

	// Variable Defaults
	$member  = new TravelAffiliateMembership();
	$subject = sprintf("Password Changed for %s Affiliate Membership", SITE_NAME);
	...
	if (password_verify(filter_input(INPUT_POST, 'old_password'), $member->getPasswordHash())) {
		...
		$mailer->setBody('travel-affiliate-members/password/change/notification.twig', array(
		...
		$json_response = array(
			'status' => 'success',
			'html'   => Template::Render('travel-affiliate-members/password/change/success.twig')
		);

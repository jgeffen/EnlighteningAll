	...
	*/
	...
	try {
		...
		$form_values = $authnetClient->getTransactionDetails(array(
			...
			'type'                    => $product->isTip() ? 'tips' : $authnetClient->getType(),
			...
			'invoice'                 => Client::GenerateInvoice('AN', $product->getId(), ...$authnetClient->getBilling()->toArray()),
			...
			'response_text'           => $authnetClient->getTransaction()->getCvvResultCode(),
		));

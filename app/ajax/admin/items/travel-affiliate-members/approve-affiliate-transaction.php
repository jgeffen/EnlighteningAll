...
try {
    ...
    $errors = call_user_func(function () {
        // Variable Defaults
        $required = array(
            'admin_approved' => FILTER_VALIDATE_INT,
    ...
    // Update Database
    Database::Action(
        "UPDATE `affiliate_transactions` SET `admin_approved` = :admin_approved WHERE `transaction_id` = :transaction_id",
        array(
            'admin_approved' => filter_input(INPUT_POST, 'admin_approved'),
    ...
    $json_response = array(
        'status' => 'success',
        'message' => 'affiliate_transations table has been updated successfully with admin approval or disapproval',
    );

...
try {
    ...
    // Update Database
    Database::Action(
        "UPDATE `affiliate_transactions` SET `is_paid` = :is_paid WHERE `transaction_id` = :transaction_id",
        array(
            'is_paid' => filter_input(INPUT_POST, 'is_paid'),
    ...
    $json_response = array(
        'status' => 'success',
        'message' => 'affiliate_transations table has been updated successfully with admin paid',
    );

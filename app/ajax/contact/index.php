<?php
	...
	*/
	...
	try {
		// Variable Defaults
		$subject  = sprintf("Contact Request - %s", SITE_NAME);
		...
		$mailer->setBody('forms/contact/notification.twig', array(
		...
		// Update Database
		Database::Action("INSERT INTO `forms` SET `type` = :type, `address` = :address, `captcha` = :captcha, `city` = :city, `comments` = :comments, `email` = :email, `name` = :name, `phone` = :phone, `state` = :state, `status` = :status, `zip_code` = :zip_code, `original` = :original, `user_agent` = :user_agent, `ip_address` = :ip_address, `timestamp` = :timestamp", array(
			'type'       => 'contact',
			'address'    => filter_input(INPUT_POST, 'address'),

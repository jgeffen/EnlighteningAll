...
try {
    ...
    $errors = call_user_func(function () {
        ...
        foreach ($required as $field => $validation) {
            switch ($validation) {
                case FILTER_VALIDATE_INT:
                    if (is_null(filter_input(INPUT_POST, $field, $validation)) ||
                        filter_input(INPUT_POST, $field, $validation) === FALSE) {
        ...
        if (filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL)) {
            if (Membership::EmailExists(filter_input(INPUT_POST, 'email', FILTER_VALIDATE_EMAIL))) {
        ...
        if (filter_input(INPUT_POST, 'username')) {
            if (!Membership::UsernameExists(filter_input(INPUT_POST, 'username'))) {
    ...
    if (is_null($member)) {
        throw new Exception('Something went wrong with your registration. Please try again.');
    }
    ...
    try {
        ...
        if ($member->settings()->getValue('avatar_approval_automatic')) {
            if ($member->isApproved() ||
                !$member->settings()->getValue('account_approval_required_avatar_approval')) {
                $auto_approve = TRUE;
            }
        }

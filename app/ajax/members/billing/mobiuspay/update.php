<?php
	...
	*/
	...
	try {
		...
		$mobiusPayVault->setBillingId($member->wallet()->getBillingId());
		$mobiusPayVault->setShippingId($member->wallet()->getShippingId());
		
		// Set Credit Card
		$mobiusPayVault->setCreditCard(
			account    : filter_input(INPUT_POST, 'cc_number'),
			type       : filter_input(INPUT_POST, 'cc_type'),
			expiration : update . phpfilter_input(INPUT_POST, 'cc_expiry_month') . filter_input(INPUT_POST, 'cc_expiry_year'),
		...
		$mobiusPayVault->setOrder(array(
			...
			'description' => 'Customer Updated by enlighteningall.com'
		));
		
		// Process Transaction
		$mobiusPayVault->setType(MobiusPay\Vault::TYPE_UPDATE_CUSTOMER)->doTransaction();
		...
		switch($mobiusPayVault->getTransaction('response')) {
			case MobiusPay\Client::RESPONSE_APPROVED:
				...
				$member->log()->setData(
					...
					table_id   : Database::ArrayUpdate('member_wallets', array(

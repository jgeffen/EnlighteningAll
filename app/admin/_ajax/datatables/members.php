<?php
	...
	*/
	...
	echo json_encode(array(
		...
		}, Database::Action("SELECT CONCAT_WS(' ', `members`.`first_name`, `members`.`last_name`) AS `full_name`, CONCAT_WS(', ', `members`.`last_name`, `members`.`first_name`) AS `full_name_last`, CONCAT('/', `members`.`id`, '.profile') AS `profile_link`, MAX(`member_logs`.`timestamp`) AS `last_logged_in`, `members`.* FROM `members` LEFT JOIN `member_logs` ON (`member_logs`.`member_id` = `members`.`id` AND `member_logs`.`action` IN ('Login')) GROUP BY `members`.`id`")->fetchAll(PDO::FETCH_ASSOC))
	));
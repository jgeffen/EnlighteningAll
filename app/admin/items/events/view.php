...
<main class="page-content">
	<section id="view-table" role="region">
		<div id="page-title-btn">
			...
			<a class="btn btn-primary" href="/user/add/events">
		...
		<span class="d-block mb-3">
			...
			<button data-action="published" class="badge badge-ev-light">Published</button>
			<button data-action="unpublished" class="badge bg-warning text-dark">Unpublished</button>
	...
	$(function() {
		...
		tableElement.DataTable({
            ...
            columns: [
                ...
                {
                    data: 'sort_order',           // ✅ new field for order position
                    className: 'all text-center', // optional: centers numbers
                    title: 'Order'                // optional: column header title
                },
                {
                    data: {
                        _: 'item.page_title',
						...
						switch (action) {
							case 'delete':
								...
								if (confirm('Are you sure you want to delete this?')) {
									// Handle Ajax
									$.ajax('/user/delete/events/' + data.item.id, {
										dataType: 'json',
										method: 'delete',
										async: true,
		...
		$(this.body).on('click', 'button[data-action]', function() {
			...
			switch ($(this).data("action")) {
				...
				case "unpublished":
					...
					table.column(2).search('false').draw();
					break;

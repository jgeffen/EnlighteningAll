<?php
	...
	*/
	...
	try {
		...
		$item     = Secrets\Photographers\Package::Init($item['id']);
		...
		$errors   = call_user_func(function() use ($page_url) {
			// Required Fields
			$required = array(
				'photographer_id' => FILTER_VALIDATE_INT,
		...
		// Update Database
		Database::Action("UPDATE `photographer_packages` SET `photographer_id` = :photographer_id, `page_title` = :page_title, `page_description` = :page_description, `heading` = :heading, `price` = :price, `content` = :content, `youtube_id` = :youtube_id, `filename` = :filename, `filename_alt` = :filename_alt, `page_url` = :page_url, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(
		...
		// Set Route(s)
		$route        ??= Router\Route::Init('photographer_packages', $item->getId());
		$parent_route ??= Router\Route::Init('photographers', filter_input(INPUT_POST, 'photographer_id', FILTER_VALIDATE_INT));
		$parent_route ??= Router\Route::Init('photographer_packages');
		...
		if(is_null($route)) {
			...
			// Update Routes
			Database::Action("UPDATE `routes` SET `parent_route_id` = :parent_route_id, `table_name` = :table_name, `table_id` = :table_id, `page_url` = :page_url, `category` = :category, `categories` = :categories, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(
				'parent_route_id' => $parent_route?->getId(),
				'table_name'      => 'photographer_packages',
				'table_id'        => $item->getId(),
				'page_url'        => $page_url,

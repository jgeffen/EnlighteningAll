<?php
	...
	*/
	...
	// Variable Defaults
	$page_title  = 'Edit Photographer Package';
	$image_sizes = array(
		'landscape' => array(
			'width'  => 900,
			'height' => 600
	...
	// Set Item
	$item = Secrets\Photographers\Package::Fetch(Database::Action("SELECT * FROM `photographer_packages` WHERE `id` = :table_id", array(
	...
	// Set Photographers
	$photographers = Database::Action("SELECT `id`, `name` FROM `photographers` ORDER BY `name`")->fetchAll(PDO::FETCH_COLUMN | PDO::FETCH_UNIQUE);
	...
	<div id="ajax-wrapper">
		<form class="form-horizontal content-module">
			<div class="form-group">
				<label for="photographers-id">Photographer</label>
				
				<div class="select-wrap form-control">
					<select id="photographers-id" name="photographer_id" data-value="<?php echo $item->getPhotographerId(); ?>">
						<?php foreach($photographers as $value => $label): ?>
								...
								<?php foreach($image_sizes as $type => $format): ?>
									<div class="col-lg-6 col-xl-4">
										<a class="image-cropper-crop" href="#" data-cropper-action="crop">
											...
											<img class="img-fluid"
												src="<?php echo sprintf("/files/photographer_packages/%s/%s?v=%d", $type, $item->getFilename(), time()); ?>"
			...
			<div class="form-btns text-right">
				<div class="float-lg-right">
					...
					<a class="btn btn-danger btn-block-md ml-lg-1 mb-2" href="/user/view/photographer-packages">
...
<script>
	$(function() {
		...
		$('#image-cropper-wrapper').cropper({
			...
			additionalData: { table_name: 'photographer_packages', sizes: imageSizes, column: 'filename', item: item },
			...
			$.ajax({
				...
				success: function(response) {
					...
					switch(response.status) {
						case 'success':
							location.href = '/user/view/photographer-packages';
							break;
						case 'error':

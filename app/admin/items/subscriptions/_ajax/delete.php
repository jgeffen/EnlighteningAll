<?php
	...
	*/
	...
	// TODO: Check if subscription has members
	...
	try {
		...
		// Fetch Item
		$item = Items\Subscription::Init($dispatcher->getTableId());
		
		// Check Required Data
		if(empty($item)) throw new Exception('Item not found in database.');
		
		// Update Database
		Database::Action("DELETE FROM `subscriptions` WHERE `id` = :id", array('id' => $item->getId()));

<?php
	...
	*/
	...
	try {
		// Validate Form
		$page_url = Helpers::FormatPageURL(filter_input(INPUT_POST, 'question'), TRUE);
		$errors   = call_user_func(function() {
			// Required Fields
			$required = array(
				'category_id'    => FILTER_VALIDATE_INT,
				'question'       => FILTER_DEFAULT,
				'answer'         => FILTER_DEFAULT,
		...
		// Update Database
		$table_id = Database::Action("INSERT INTO `member_faqs` SET `category_id` = :category_id, `question` = :question, `answer` = :answer, `page_url` = :page_url, `position` = :position, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			'category_id'    => filter_input(INPUT_POST, 'category_id', FILTER_VALIDATE_INT),
			'question'       => filter_input(INPUT_POST, 'question'),
			'answer'         => filter_input(INPUT_POST, 'answer'),
			'page_url'       => $page_url,
			'position'       => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `member_faqs`")->fetch(PDO::FETCH_COLUMN),
		...
		$admin->log(
			...
			table_name : Tables\Members::FAQS,
			table_id   : $table_id,
			payload    : $_POST
		);

<?php
	...
	*/
	...
	try {
		...
		$item     = Members\Faq::Init((int)$item['id']);
		$page_url = Helpers::FormatPageURL(filter_input(INPUT_POST, 'page_url') ?: filter_input(INPUT_POST, 'question'), TRUE, $item?->getPageUrl());
		...
		// Update Database
		Database::Action("UPDATE `member_faqs` SET `category_id` = :category_id, `question` = :question, `answer` = :answer, `page_url` = :page_url, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(

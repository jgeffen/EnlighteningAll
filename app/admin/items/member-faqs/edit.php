<?php
	...
	*/
	...
	// Variable Defaults
	$page_title = 'Edit Members FAQ';
	
	// Set Item
	$item = Members\Faq::Fetch(Database::Action("SELECT * FROM `member_faqs` WHERE `id` = :table_id", array(
	...
	// Set Categories
	$categories = Database::Action("SELECT `id`, `name` FROM `categories` WHERE `table_name` = :table_name ORDER BY `name`", array(
		...
		<form class="form-horizontal content-module">
			...
			<div class="form-group">
				...
				<div class="feedback-wrap">
					<textarea id="question" class="form-control disable-mce" name="question" rows="5"><?php echo $item->getQuestion(); ?></textarea>
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="feedback-wrap">
					<textarea id="answer" class="form-control" name="answer" rows="20"><?php echo $item->getAnswer(); ?></textarea>
				</div>
			</div>
			...
			<div class="form-btns text-right">
				<div class="float-lg-right">
					...
					<a class="btn btn-danger btn-block-md ml-lg-1 mb-2" href="/user/view/member-faqs">
			...
			$.ajax({
				...
				success: function(response) {
					...
					switch(response.status) {
						case 'success':
							location.href = '/user/view/member-faqs';
							break;
						case 'error':

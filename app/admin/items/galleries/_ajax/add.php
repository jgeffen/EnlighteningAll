	...
	*/
	...
	try {
		...
		$table_id = Database::Action("INSERT INTO `galleries` SET `category_id` = :category_id, `page_title` = :page_title, `page_description` = :page_description, `heading` = :heading, `content` = :content, `youtube_id` = :youtube_id, `filename` = :filename, `filename_alt` = :filename_alt, `page_url` = :page_url, `position` = :position, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			...
			'page_url'         => $page_url,
			'position'         => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `galleries`")->fetch(PDO::FETCH_COLUMN),

<?php
	...
	*/
	...
	try {
		...
		// Update Database
		$table_id = Database::Action("INSERT INTO `faqs` SET `category_id` = :category_id, `question` = :question, `answer` = :answer, `position` = :position, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			'category_id'    => filter_input(INPUT_POST, 'category_id', FILTER_VALIDATE_INT),
			'question'       => filter_input(INPUT_POST, 'question'),
			'answer'         => filter_input(INPUT_POST, 'answer'),
			'position'       => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `faqs`")->fetch(PDO::FETCH_COLUMN),
		...
		$admin->log(
			...
			table_name : Tables\Website::FAQS,
			table_id   : $table_id,
			payload    : $_POST
		);

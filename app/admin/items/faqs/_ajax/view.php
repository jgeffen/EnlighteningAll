	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Items\Faq $item) => array(
				'id'        => $item->getId(),
				...
				'item'      => $item->toArray(),
				'options'   => Render::GetTemplate('admin/items/faqs/options.twig', array('id' => $item->getId()))
			), Items\Faq::FetchAll(Database::Action("SELECT * FROM `faqs` ORDER BY `position` DESC")))
		);

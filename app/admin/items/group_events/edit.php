<?php
	...
	*/
	...
	// Set Item
	$item = Items\GroupEvent::Fetch(Database::Action("SELECT * FROM `group_events` WHERE `id` = :table_id", array(
								...
								<?php foreach($image_sizes as $type => $format): ?>
									<div class="col-lg-6 col-xl-4">
										<a class="image-cropper-crop" href="#" data-cropper-action="crop">
											...
											<img class="img-fluid"
												src="<?php echo sprintf("/files/group_events/%s/%s?v=%d", $type, $item->getFilename(), time()); ?>"
...
<script>
	$(function() {
		...
		$('#image-cropper-wrapper').cropper({
			...
			additionalData: { table_name: 'group_events', sizes: imageSizes, column: 'filename', item: item },

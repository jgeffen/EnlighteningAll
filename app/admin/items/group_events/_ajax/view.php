	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Items\GroupEvent $item) => array(
				'id'         => $item->getId(),
				...
				'item'       => $item->toArray(),
				'options'    => Render::GetTemplate('admin/items/group_events/options.twig', array('id' => $item->getId()))
			), Items\GroupEvent::FetchAll(Database::Action("SELECT * FROM `group_events` ORDER BY `position` DESC")))
		);

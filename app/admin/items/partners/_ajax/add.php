<?php
	...
	*/
	...
	try {
		...
		// Update Database
		$table_id = Database::Action("INSERT INTO `partners` SET `heading` = :heading, `content` = :content, `link` = :link, `filename` = :filename, `filename_alt` = :filename_alt, `position` = :position, `published` = :published, `published_date` = :published_date, `analytics` = :analytics, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			'heading'        => filter_input(INPUT_POST, 'heading'),
			'content'        => filter_input(INPUT_POST, 'content'),
			'link'           => filter_input(INPUT_POST, 'link'),
			'filename'       => filter_input(INPUT_POST, 'filename'),
			'filename_alt'   => filter_input(INPUT_POST, 'filename_alt'),
			'position'       => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `partners`")->fetch(PDO::FETCH_COLUMN),

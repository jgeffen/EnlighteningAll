	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Items\Podcast $item) => array(
				'id'         => $item->getId(),
				...
				'item'       => $item->toArray(),
				'options'    => Render::GetTemplate('admin/items/podcasts/options.twig', array('id' => $item->getId()))
			), Items\Podcast::FetchAll(Database::Action("SELECT * FROM `podcasts` ORDER BY `position` DESC")))
		);

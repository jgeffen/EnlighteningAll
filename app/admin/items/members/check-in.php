...
<main class="page-content">
	<section id="view-table" role="region">
		...
		<div id="check-in-form" class="row">
			<div class="col-12">
...
<script>
	$(function() {
		...
		form.on('submit', function(event) {
			...
			// Handle Ajax Request
			$.ajax('/ajax/admin/items/members/check-in/fetch-account', {
				...
				success: function(response) {
					...
					switch(response.status) {
						case 'success':
							// Replace Content
							ajaxWrapper.html(response.html).on('click', '[data-check-in-action]', function(event) {
								// Prevent Default
								event.preventDefault();
								...
								var action    = dataset.checkInAction;
								...
								switch(action) {
									...
									case 'confirm':
										// Handle Ajax Request
										$.ajax('/ajax/admin/items/members/check-in/confirm/' + member_id, {
											data: { expiration: wrapper.find('select[name="expiration"]').val() },
											dataType: 'json',
											async: false,
											method: 'post',
											beforeSend: showLoader,
									...
									default:
										console.log('Unknown Check-In Action: ', action);
								}
							});

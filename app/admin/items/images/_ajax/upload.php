<?php
	...
	*/
	...
	try {
		...
		$image_sizes = json_decode(filter_input(INPUT_POST, 'sizes'), TRUE);
		$types       = array('jpeg', 'jpg', 'png');
		
		// Check Required Data
		if(empty($image_sizes)) throw new Exception('No sizes were set.');
		...
		// Variable Defaults
		$path     = Helpers::CreateDirectory("/files/images");
		$pathinfo = pathinfo($_FILES['image']['name']);
		$position = Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `images` WHERE `table_name` = :table_name AND `table_id` = :table_id", array(
		...
		switch($_FILES['image']['error']) {
			case 0:
				...
				// Create Thumbs
				foreach($image_sizes as $type => $format) {
					// Variable Croppable
					...
					// Write Small Thumb
					$thumb->thumbnailImage($format['width'] * 0.5, $format['height'] * 0.5, $croppable);
					$thumb->writeImage(sprintf("%s/%s", Helpers::CreateDirectory(sprintf("%s/%s/thumbs", $path, $type)), $filename));
				}
				
				// Update Database
				$table_id = Database::Action("INSERT INTO `images` SET `table_name` = :table_name, `table_id` = :table_id, `filename` = :filename, `filename_alt` = :filename_alt, `position` = :position, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
					'table_name'     => $table_name,
					'table_id'       => $table_id,
					'filename'       => $filename,
					'filename_alt'   => ucwords(str_replace('-', ' ', pathinfo($filename, PATHINFO_FILENAME))),
					'position'       => $position,
					'published'      => TRUE,

<?php
	...
	*/
	...
	try {
		...
		// Update Database
		Database::Action("UPDATE `images` SET `filename_alt` = :filename_alt, `youtube_id` = :youtube_id, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(
		...
		$admin->log(
			...
			table_name : Tables\Website::IMAGES,
			table_id   : $item->getId(),
			payload    : $_POST
		);

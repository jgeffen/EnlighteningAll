<?php
	...
	*/
	...
	// Set Item
	$item = Items\Service::Fetch(Database::Action("SELECT * FROM `services` WHERE `id` = :table_id", array(
		...
		<form class="form-horizontal content-module">
								...
								<?php foreach($image_sizes as $type => $format): ?>
									<div class="col-lg-6 col-xl-4">
										<a class="image-cropper-crop" href="#" data-cropper-action="crop">
											...
											<img class="img-fluid"
												src="<?php echo sprintf("/files/services/%s/%s?v=%d", $type, $item->getFilename(), time()); ?>"
			...
			<div class="form-btns text-right">
				<div class="float-lg-right">
					...
					<a class="btn btn-danger btn-block-md ml-lg-1 mb-2" href="/user/view/services">
...
<script>
	$(function() {
		...
		$('#image-cropper-wrapper').cropper({
			...
			additionalData: { table_name: 'services', sizes: imageSizes, column: 'filename', item: item },
			...
			$.ajax({
				...
				success: function(response) {
					...
					switch(response.status) {
						case 'success':
							location.href = '/user/view/services';
							break;
						case 'error':

	...
	*/
	...
	try {
		...
		$table_id = Database::Action("INSERT INTO `products` SET `category_id` = :category_id, `label` = :label, `merchant` = :merchant, `price` = :price, `stock_quantity` = :stock_quantity, `taxable` = :taxable, `is_fridge` = :is_fridge, `page_title` = :page_title, `page_description` = :page_description, `heading` = :heading, `content` = :content, `youtube_id` = :youtube_id, `filename` = :filename, `filename_alt` = :filename_alt, `page_url` = :page_url, `position` = :position, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address, `timestamp` = :timestamp, `last_timestamp` = :last_timestamp", array(
			...
			'is_fridge'        => filter_input(INPUT_POST, 'is_fridge', FILTER_VALIDATE_INT, array('options' => array('default' => 0))),
			...
			'page_url'         => $page_url,
			'position'         => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `products`")->fetch(PDO::FETCH_COLUMN),

<?php
	...
	*/
	...
	try {
		...
		// Update Database
		$table_id = Database::Action("INSERT INTO `sliders` SET `page_url` = :page_url, `heading` = :heading, `content` = :content, `content_position` = :content_position, `link` = :link, `link_text` = :link_text, `filename` = :filename, `filename_alt` = :filename_alt, `position` = :position, `delete_on_expiration` = :delete_on_expiration, `published` = :published, `expiration_date` = :expiration_date, `published_date` = :published_date, `analytics` = :analytics, `expiration` = :expiration, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			...
			'position'             => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `sliders`")->fetch(PDO::FETCH_COLUMN),

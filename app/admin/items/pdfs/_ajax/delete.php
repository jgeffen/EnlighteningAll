<?php
	...
	*/
	...
	try {
		...
		// Fetch Item
		$item = Items\PDF::Fetch(Database::Action("SELECT `id`, `table_name`, `table_id`, `filename` FROM `pdfs` WHERE `id` = :id AND `table_name` = :table_name AND `table_id` = :table_id", array(
		...
		// Remove File
		Admin\File::Remove($item->getFilename(), Tables\Website::PDFS, $item->getId(), NULL);
		
		// Update Database
		Database::Action("DELETE FROM `pdfs` WHERE `id` = :id", array('id' => $item->getId()));

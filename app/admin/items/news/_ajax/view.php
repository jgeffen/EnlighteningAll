	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Items\News $item) => array(
				'id'         => $item->getId(),
				...
				'item'       => $item->toArray(),
				'options'    => Render::GetTemplate('admin/items/news/options.twig', array('id' => $item->getId()))
			), Items\News::FetchAll(Database::Action("SELECT * FROM `news` ORDER BY `position` DESC")))
		);

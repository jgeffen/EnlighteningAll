...
try {
	...
	$json_response = array(
		...
		'data'    => array_map(function (TravelAffiliateMembership $item) {
			return array(
				...
				'username'       => array(
					...
					'label' => Render::GetTemplate('admin/items/travel-affiliate-members/username.twig', array(
				...
				'item'           => $item->toArray(),
				'options'        => Render::GetTemplate('admin/items/travel-affiliate-members/options.twig', array(
					'id'    => $item->getId(),
		...
		}, TravelAffiliateMembership::FetchAll(Database::Action("SELECT * FROM `travel_affiliate_members` WHERE `is_employee` = 0 ORDER BY `last_name`, `first_name`")))
	);

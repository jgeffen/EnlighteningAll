<?php
	...
	*/
	...
	try {
		...
		$to_email = SITE_EMAIL;
		$subjects = array(
			'success' => sprintf("Booking Void Success - %s", SITE_NAME),
			'failure' => sprintf("Booking Void Failure - %s", SITE_NAME)
		);
		...
		// Process Transaction
		$mobiusPayClient->setType(MobiusPay\Client::TYPE_VOID)->doTransaction();
		...
		switch($mobiusPayClient->getTransaction('response')) {
			case MobiusPay\Client::RESPONSE_APPROVED:
				...
				$mailer->setBody(sprintf("admin/items/transactions/forms/%s/void/notifications/success.twig", $transaction->getForm()), match ($transaction->getForm()) {
				...
				// Set Message
				Admin\SetMessage('Voided transaction successfully.', 'success');
			...
			case MobiusPay\Client::RESPONSE_ERROR:
				...
				$mailer->setBody(sprintf("admin/items/transactions/forms/%s/void/notifications/declined.twig", $transaction->getForm()), match ($transaction->getForm()) {
					'photographer-package-booking' => array(

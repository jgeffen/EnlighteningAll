<?php
	...
	*/
	...
	try {
		...
		$to_email = SITE_EMAIL;
		$subjects = array(
			'success' => sprintf("Booking Refund Success - %s", SITE_NAME),
			'failure' => sprintf("Booking Refund Failure - %s", SITE_NAME),
			'receipt' => sprintf("Booking Refund Receipt - %s", SITE_NAME)
		);
		...
		// Process Transaction
		$mobiusPayClient->setType(MobiusPay\Client::TYPE_REFUND)->doTransaction($amount);
		...
		switch($mobiusPayClient->getTransaction('response')) {
			case MobiusPay\Client::RESPONSE_APPROVED:
				...
				$mailer->setBody(sprintf("admin/items/transactions/forms/%s/refund/notifications/success.twig", $transaction->getForm()), match ($transaction->getForm()) {
				...
				if(filter_input(INPUT_POST, 'email_receipt', FILTER_VALIDATE_INT)) {
					...
					$mailer->setBody(sprintf("admin/items/transactions/forms/%s/refund/receipt.twig", $transaction->getForm()), match ($transaction->getForm()) {
						'photographer-package-booking' => array(
				...
				// Set Message
				Admin\SetMessage('Refunded transaction successfully.', 'success');
			...
			case MobiusPay\Client::RESPONSE_ERROR:
				...
				$mailer->setBody(sprintf("admin/items/transactions/forms/%s/refund/notifications/declined.twig", $transaction->getForm()), match ($transaction->getForm()) {
					'photographer-package-booking' => array(

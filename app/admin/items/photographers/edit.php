<?php
	...
	*/
	...
	// Set Categories
	if(Admin\Categories('photographers')) {
	...
	<div id="ajax-wrapper">
		<form class="form-horizontal content-module">
			...
			<div class="form-group">
				...
				<div class="feedback-wrap">
					<input id="name" class="form-control" type="text" name="name" maxlength="75" value="<?php echo $item->getEncoded('name'); ?>">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="feedback-wrap">
					<input id="email" class="form-control" type="email" name="email" maxlength="75" value="<?php echo $item->getEncoded('email'); ?>">
				</div>
			</div>
			...
			<div class="form-group">
				...
				<div class="feedback-wrap">
					<input id="phone" class="form-control" type="text" name="phone" maxlength="75" data-format="phone" value="<?php echo $item->getEncoded('phone'); ?>">
				</div>
			</div>
								...
								<?php foreach($image_sizes as $type => $format): ?>
									<div class="col-lg-6 col-xl-4">
										<a class="image-cropper-crop" href="#" data-cropper-action="crop">
											...
											<img class="img-fluid"
												src="<?php echo sprintf("/files/photographers/%s/%s?v=%d", $type, $item->getFilename(), time()); ?>"
			...
			<div class="form-btns text-right">
				<div class="float-lg-right">
					...
					<a class="btn btn-danger btn-block-md ml-lg-1 mb-2" href="/user/view/photographers">
...
<script>
	$(function() {
		...
		$('#image-cropper-wrapper').cropper({
			...
			additionalData: { table_name: 'photographers', sizes: imageSizes, column: 'filename', item: item },
			...
			$.ajax({
				...
				success: function(response) {
					...
					switch(response.status) {
						case 'success':
							location.href = '/user/view/photographers';
							break;
						case 'error':

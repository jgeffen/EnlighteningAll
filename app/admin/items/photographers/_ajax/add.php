<?php
	...
	*/
	...
	try {
		// Variable Defaults
		$page_url = Helpers::FormatPageURL(filter_input(INPUT_POST, 'name'), TRUE);
		...
		$table_id = Database::Action("INSERT INTO `photographers` SET `category_id` = :category_id, `page_title` = :page_title, `page_description` = :page_description, `heading` = :heading, `name` = :name, `email` = :email, `phone` = :phone, `content` = :content, `youtube_id` = :youtube_id, `filename` = :filename, `filename_alt` = :filename_alt, `page_url` = :page_url, `position` = :position, `published` = :published, `published_date` = :published_date, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			...
			'page_url'         => $page_url,
			'position'         => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `photographers`")->fetch(PDO::FETCH_COLUMN),

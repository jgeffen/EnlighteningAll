<?php
	...
	*/
	...
	try {
		...
		$item     = Secrets\Photographer::Init($item['id']);
		...
		$admin->log(
			...
			table_name : Tables\Secrets::PHOTOGRAPHERS,
			table_id   : $item->getId(),
			payload    : $_POST
		);
		
		// Set Route(s)
		$route        ??= Router\Route::Init('photographers', $item->getId());
		$parent_route ??= Router\Route::Init('categories', filter_input(INPUT_POST, 'category_id', FILTER_VALIDATE_INT));
		$parent_route ??= Router\Route::Init('photographers');

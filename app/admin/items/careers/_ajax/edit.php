<?php
	...
	*/
	...
	try {
		...
		$item     = Items\Career::Init($item['id']);
		...
		$admin->log(
			...
			table_name : Tables\Website::CAREERS,
			table_id   : $item->getId(),
			payload    : $_POST
		);
		
		// Set Route(s)
		$route        ??= Router\Route::Init('careers', $item->getId());
		$parent_route ??= Router\Route::Init('categories', filter_input(INPUT_POST, 'category_id', FILTER_VALIDATE_INT));
		$parent_route ??= Router\Route::Init('careers');
		...
		if(is_null($route)) {
			...
			$route_id = Database::Action("INSERT INTO `routes` SET `parent_route_id` = :parent_route_id, `table_name` = :table_name, `table_id` = :table_id, `page_url` = :page_url, `category` = :category, `categories` = :categories, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
				'parent_route_id' => filter_input(INPUT_POST, 'category_id', FILTER_VALIDATE_INT),
				'table_name'      => 'careers',
				'table_id'        => $item->getId(),
				'page_url'        => $page_url,
				'category'        => FALSE,

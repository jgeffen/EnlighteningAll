	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Items\Career $item) => array(
				'id'         => $item->getId(),
				...
				'item'       => $item->toArray(),
				'options'    => Render::GetTemplate('admin/items/careers/options.twig', array('id' => $item->getId()))
			), Items\Career::FetchAll(Database::Action("SELECT * FROM `careers` ORDER BY `position` DESC")))
		);

<?php
	...
	*/
	...
	try {
		...
		$item = Posts\Report::Init($item['id']);
		
		// Check Item
		if(is_null($item)) throw new Exception('Item not found in database.');
		
		// Check Post
		if(is_null($item->getPost())) throw new Exception('Post no longer exists in database.');
		...
		// Update Database
		Database::Action("UPDATE `member_post_reports` SET `status` = :status, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(
		...
		$admin->log(
			...
			table_name : Tables\Members\Posts::REPORTS,
			table_id   : $item->getId(),
			payload    : $_POST,
		...
		if(Statuses\Report::lookup(filter_input(INPUT_POST, 'status'))?->is(Statuses\Report::REMOVED)) {
			// Set Directory
			$directory = sprintf("%s/files/members/%d", filter_input(INPUT_SERVER, 'DOCUMENT_ROOT'), $item->getPost()->getMemberId());
			
			// Remove Image
			Helpers::RemoveFile($directory, $item->getPost()->getFilename());
			
			// Update Database
			Database::Action("DELETE FROM `member_posts` WHERE `id` = :id AND `member_id` = :member_id", array(
				'id'        => $item->getPost()->getId(),
				'member_id' => $item->getPost()->getMemberId()
			));

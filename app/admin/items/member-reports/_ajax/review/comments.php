<?php
	...
	*/
	...
	try {
		...
		$item = Comments\Report::Init($item['id']);
		...
		// Update Database
		Database::Action("UPDATE `member_post_comment_reports` SET `status` = :status, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(
		...
		$admin->log(
			...
			table_name : Tables\Members\Posts\Comments::REPORTS,
			table_id   : $item->getId(),
			payload    : $_POST,
		...
		if(Statuses\Report::lookup(filter_input(INPUT_POST, 'status'))?->is(Statuses\Report::REMOVED)) {
			// Update Database
			Database::Action("DELETE FROM `member_post_comments` WHERE `id` = :id", array(
			...
			$admin->log(
				...
				table_name : Tables\Members::COMMENTS,
				table_id   : $item->getId(),
				payload    : $_POST,
				notes      : filter_input(INPUT_POST, 'notes')
			);

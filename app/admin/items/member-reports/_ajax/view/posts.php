	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Posts\Report $item) => array(
				'status'      => $item->getStatus()?->getValue(),
				'type'        => $item->getType()?->getValue(),
				'id'          => $item->getId(),
				'post_id'     => $item->getPostId(),
				'posted_by'   => $item->getPost()?->getMember()?->getUsername(),
				'reported_by' => $item->getMember()?->getUsername(),
				'timestamp'   => $item->getLastTimestamp()->format('F j, Y, g:ia'),
				'item'        => $item->toArray(),
				'options'     => Render::GetTemplate('admin/items/member-reports/posts/options.twig', array(
					'id' => $item->getId()
				))
			), Posts\Report::FetchAll(Database::Action("SELECT * FROM `member_post_reports` ORDER BY `timestamp`")))
		);

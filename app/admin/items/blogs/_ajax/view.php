	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'    => array_map(fn(Items\Blog $item) => array(
				'id'         => $item->getId(),
				...
				'item'       => $item->toArray(),
				'options'    => Render::GetTemplate('admin/items/blogs/options.twig', array('id' => $item->getId()))
			), Items\Blog::FetchAll(Database::Action("SELECT * FROM `blogs` ORDER BY `position` DESC")))
		);

	...
	*/
	...
	try {
		...
		$json_response = array(
			...
			'data'       => array_map(fn(Items\Members\Log $item) => array(
				'id'                 => $item->getId(),
			...
			), Items\Members\Log::FetchAll(Database::Action("SELECT * FROM `member_logs` WHERE `member_id` = :member_id ORDER BY `timestamp` DESC", array(
				'member_id' => $member->getId()
			)))),
			'categories' => call_user_func(fn($categories) => !$categories['TABLE NAMES'] ? array() : array(
				'data'   => $categories['TABLE NAMES'],
				'html'   => Render::GetTemplate('admin/items/members/logs/categories.twig', array('categories' => $categories)),
				'filter' => 'table_name'
			), array(
				'DEFAULTS'    => array('default.show_all' => 'SHOW ALL'),
				'ACTIONS'     => Database::Action("SELECT CONCAT('action.', `type`) AS `value`, UPPER(`type`) AS `text` FROM `member_logs` GROUP BY `type` ORDER BY `type`")->fetchAll(PDO::FETCH_COLUMN | PDO::FETCH_UNIQUE),
				'TABLE NAMES' => Database::Action("SELECT `table_name` AS `value`, UPPER(REPLACE(REPLACE(`table_name`, 'member_', ''), '_', ' ')) AS `text` FROM `information_schema`.`tables` WHERE `table_schema` = :table_schema AND LOCATE('member_', `table_name`) ORDER BY `table_name`", array(

...
<main class="page-content">
	<section id="view-table" role="region">
		<div id="page-title-btn">
			...
			<a class="btn btn-primary" href="/user/add/sponsors">
		...
		<span class="d-block mb-3">
			...
			<span class="badge bg-info text-light">Has Script</span>
		</span>
	...
	$(function() {
		...
		tableElement.DataTable({
			...
			columns: [
				...
				{ data: { _: 'item.heading', display: 'heading' }, className: 'all' },
				...
				details: {
					display: $.fn.dataTable.Responsive.display.modal({
						header: function(row) {
							...
							return 'Details for Sponsor ID #' + data.id;
						}
					}),
			...
			rowCallback: function(row) {
				...
				!dataTable.row(row).data().script && tableRow.addClass('table-info');
						...
						switch(action) {
							case 'delete':
								...
								if(confirm('Are you sure you want to delete this?')) {
									// Handle Ajax
									$.ajax('/user/delete/sponsors/' + data.item.id, {
										dataType: 'json',
										method: 'delete',
										async: true,

<?php
	...
	*/
	...
	try {
		...
		// Update Database
		$table_id = Database::Action("INSERT INTO `sponsors` SET `heading` = :heading, `content` = :content, `link` = :link, `script` = :script, `filename` = :filename, `filename_alt` = :filename_alt, `position` = :position, `published` = :published, `published_date` = :published_date, `analytics` = :analytics, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address", array(
			...
			'position'       => Database::Action("SELECT IFNULL(MAX(`position`), 0) + 1 FROM `sponsors`")->fetch(PDO::FETCH_COLUMN),

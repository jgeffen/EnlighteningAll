<?php
	...
	*/
	...
	try {
		...
		$item   = Items\Sponsor::Init($item['id']);
		...
		// Update Database
		Database::Action("UPDATE `sponsors` SET `heading` = :heading, `content` = :content, `link` = :link, `script` = :script, `filename` = :filename, `filename_alt` = :filename_alt, `published` = :published, `published_date` = :published_date, `analytics` = :analytics, `author` = :author, `user_agent` = :user_agent, `ip_address` = :ip_address WHERE `id` = :id", array(

<?php
	...
	*/
	...
	// Set Review
	$reviews = $item->getReviews($member);
				...
				<?php if(!empty($reviews)): ?>
					<?php foreach($reviews as $review): ?>
						<div class="title-bar-trim-combo">
							<div class="title-bar">
								<img src="<?php echo Items\Defaults::AVATAR_XS; ?>" data-src="<?php echo $review->getMember()->getAvatar()?->getImage(Sizes\Avatar::XS); ?>">
								<h2 class="text-truncate">
									<a href="<?php echo $review->getMember()->getLink(); ?>" title="<?php echo $review->getMember()->getAlt('Profile'); ?>">
										<?php echo sprintf('@%s', $review->getMember()->getUsername()); ?>
									</a>
								</h2>
							</div>
							
							<div class="trim p-lg-4">
								<?php echo $review->getContent(); ?>
							</div>
						</div>
					<?php endforeach; ?>
				<?php else: ?>
					<p>Your friends have not posted any reviews. Why not be the first one to <a href="<?php echo $item->getReviewLink(); ?>">write a review</a>?</p>
				<?php endif; ?>
			</div>
		</div>
	</div>

<?php
	...
	*/
	...
	$photoLimit  = Database::Action("SELECT `private_photos_limit` FROM `member_photos_limit` WHERE `member_id` = :member_id", array(
		'member_id' => $member->getId()
	))->fetchColumn();

	// Search Engine Optimization
	$page_title       = "Social Post: Management";
	$page_description = "";
			...
			<div class="col">
				<h1 class="title-underlined mb-5">
					...
					<?php if($member->subscription()?->isPaid()):?>
						...
						<a class="btn btn-success btn-sm ml-auto" data-toggle="modal" data-target="#set_private_photo_limit">
							+ Set Limit Private Photo <?php if($photoLimit): ?><span class="badge badge-warning"><?=$photoLimit;?> </span><?php endif;?> 
						</a>
						<a class="btn btn-success btn-sm ml-3" href="/members/posts/social/private-add">
							+ New Private Photo
						</a>
					<?php else: ?>
						<a class="btn btn-success btn-sm ml-auto" data-paid-suggestion-action="private-photo-suggestion" data-mamberID="<?php echo $member->getFirstName(); ?>">
							+ New Private Photo
						</a>
					<?php endif; ?>
					<a class="btn btn-success btn-sm ml-3" href="/members/posts/social/add">
						+ New Post
					</a>
				</h1>
									...
									<div class="<?php echo $post->isApproved() ? 'not-pending' : 'pending'; ?>">
										...
										<?php if($post->visibility == "PRIVATE"):?>
										 <div style="position: absolute;bottom: 10px;">
											<div style="bottom: 20px;" class="badge badge-danger"><?=$post->visibility;?></div>
								  		</div>
										<?php endif;?>
									</div>

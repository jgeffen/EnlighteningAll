<?php
	...
	*/
	...
	try {
		...
		$pagination->setQuery("SELECT * FROM `group_events` WHERE `published` IS TRUE ORDER BY `position` DESC");
		...
		$items     = $pagination->getItems(Items\GroupEvent::class);
	...
	$page_title       = $pagination->formatPageString("Enlightening All™ Group Events");
		...
		<div class="row">
			<div class="col">
				<h1 class="title-underlined mt-0 mb-3 mb-md-4 d-inline-block w-100">
					<?php echo $pagination->formatPageString("Enlightening All™ Group Events"); ?>
				</h1>
				...
				<?php if($paginator->getPageCount() > 1): ?>
					...
					<nav aria-label="Group_Event page navigation.">

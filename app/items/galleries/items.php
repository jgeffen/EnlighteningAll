<?php
	...
	*/
	...
	try {
		...
		$pagination->setQuery("SELECT * FROM `galleries` WHERE `published` IS TRUE ORDER BY `position` DESC");
		...
		$items     = $pagination->getItems(Items\Gallery::class);
	...
	// Check Page
	if(isset($options['page']) && $options['page'] > $paginator->getPageCount()) Render::ErrorDocument(404);
	
	// Search Engine Optimization
	$page_title       = $pagination->formatPageString("Enlightening All™ Photo Galleries");
	$page_description = $pagination->formatPageString("Enlightening All&trade;.");
		...
		<div class="row">
			<div class="col">
				<h1 class="title-underlined mt-0 mb-3 mb-md-4 d-inline-block w-100">
					<?php echo $pagination->formatPageString("Enlightening All™ Photo Galleries"); ?>
				</h1>
